...
# ---------------------------------------------------------
# MARINE — per-chromosome parallel execution (mirrors REDItools split-bam pattern)
# ---------------------------------------------------------
# MARINE requires an MD-tagged, indexed BAM. We split the MD BAM by chromosome
# (reusing the get_chrom_list checkpoint), run MARINE per chromosome with
# --contigs, join the per-chrom final_filtered_site_info.tsv files (header kept
# once), gzip the joined raw output, then derive an edit-type-filtered TSV. The
# edit type is baked into the filtered filename so that changing
# params.common.edit_type regenerates only the filter step (the TSCC profile uses
# rerun-triggers: mtime, so a params-only change would not otherwise retrigger).

# e.g. "AG" -> conversion "A>G" matched against MARINE's strand_conversion column.
_MARINE_EDIT_TYPE = config["params"]["common"]["edit_type"]
_MARINE_CONVERSION = f"{_MARINE_EDIT_TYPE[0]}>{_MARINE_EDIT_TYPE[1]}"


rule split_marine_md_bam_by_chrom:
    """Extract one chromosome from the MD-tagged BAM for parallel MARINE processing."""
    input:
        bam="results/mapped/{aligner}/{condition}_{sample}.rmdup_MD.bam",
        bai="results/mapped/{aligner}/{condition}_{sample}.rmdup_MD.bam.bai"
    output:
        bam=temp("results/mapped/{aligner}/{condition}_{sample}.split_md/{chrom}.bam"),
        bai=temp("results/mapped/{aligner}/{condition}_{sample}.split_md/{chrom}.bam.bai")
    wildcard_constraints:
        chrom="[^/]+"
    resources:
        mem_mb=lambda wildcards, attempt: 2000 * (1.5 ** (attempt - 1)),
    ...
    container: container_for("wgs")
    log:
        stdout="results/logs/{aligner}_{condition}_{sample}_{chrom}.split_md_bam.out",
        stderr="results/logs/{aligner}_{condition}_{sample}_{chrom}.split_md_bam.err"
    shell:
        r"""
        set -euo pipefail
        mkdir -p "$(dirname {output.bam})"
...
rule marine_by_chrom:
    """Run MARINE on a single-chromosome MD BAM. Output is a per-chrom temp dir
    containing final_filtered_site_info.tsv."""
    input:
        bam="results/mapped/{aligner}/{condition}_{sample}.split_md/{chrom}.bam",
        bai="results/mapped/{aligner}/{condition}_{sample}.split_md/{chrom}.bam.bai",
        annotation=config["references"]["marine_annotation_bed"]
    output:
        temp(directory("results/tools/{aligner}/marine_split/{condition}_{sample}/{chrom}/"))
    wildcard_constraints:
        chrom="[^/]+"
    threads: 4
    resources:
        mem_mb=lambda wildcards, attempt: 16000 * (1.5 ** (attempt - 1)),
        runtime=lambda wildcards, attempt: 240 * (1.5 ** (attempt - 1))
    container: container_for("marine")
    log:
    ...
    shell:
        ...
        rm -rf {output}
        /opt/conda/envs/marine/bin/python /opt/marine/marine.py \
...
def _marine_chrom_dirs(wildcards):
    chrom_file = checkpoints.get_chrom_list.get(
        aligner=wildcards.aligner,
        condition=wildcards.condition,
        sample=wildcards.sample,
    ...
    chroms = [c for c in open(chrom_file).read().strip().split("\n") if c]
    return [
        f"results/tools/{wildcards.aligner}/marine_split/"
        f"{wildcards.condition}_{wildcards.sample}/{chrom}/"
        for chrom in chroms
    ]


rule join_marine_output:
    """Merge per-chromosome final_filtered_site_info.tsv files (header from first
    only) and gzip the joined raw output to save space."""
    input:
        _marine_chrom_dirs
    output:
    ...
    shell:
        ...
        first=1
        for d in {input}; do
            f="$d/final_filtered_site_info.tsv"
